...
function [z_profile,q0,zq,Q,sigma_q0,sigma_zq,sigma_Q,qz_pred,sigma_qz_pred,sigma_logqz_pred,sigma2_q0zq,...
    ...
    BSNE_profilefit(qz_profile, z_bottom_profile, z_trapheight_profile, sigma_qz_profile, sigma_z_profile, zq_estimated)
...
if nargin == 3 %if no uncertainties provided
    ...
    [~,zq] = qz_profilefit(qz_profile, z_profile, sigma_qz_profile, sigma_z_profile, zq_estimated);
end
...
if nargin == 3 %if no uncertainties provided
    ...
    [q0,zq,Q,sigma_q0,sigma_zq,sigma_Q,qz_pred,sigma_qz_pred,sigma_logqz_pred,sigma2_q0zq] = qz_profilefit(qz_profile, z_profile, sigma_qz_profile, sigma_z_profile, zq_estimated); %height-integrated flux from exponential fit
    [q0_geomean,zq_geomean,Q_geomean] = qz_profilefit(qz_profile,z_profile_geomean, sigma_qz_profile, sigma_z_profile, zq_estimated); %same thing, but using geometric mean height
end