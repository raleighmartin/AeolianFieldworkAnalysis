...
% Dependencies: GetDataTypes, GetTableTimes, ReadTable
...
%% Generate structured array, 'RawData', with hierarchical listing by instrument type then instrument then field
...
%go through each instrument time
for i = 1:N_InstrumentTypes
    ...
    for j = 1:N_Instruments
        ...
        RawData.(InstrumentType).(Instrument) = struct(...
            ...
            'InstrumentID', cellstr(InstrumentMetadata.InstrumentID(InstrumentMetadata_ind)),...    
...
for i=1:N_tables
    ...
    tableTimes = GetTableTimes(LoggerTables.Site(i),LoggerTables.Date(i),LoggerTables.Logger(i),LoggerTimes);
