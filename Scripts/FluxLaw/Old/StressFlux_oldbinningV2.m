...
tau_relative_uncertainty = 0.05; %relative uncertainty on tau values
...
% % i_Oceano = find(strncmp(Sites,'Oceano',6),1);
...
% %         fQ_std_pred = fQ_CV_Oceano(ind_Oceano)*fQ_ust_bin_avg_all{i}(k); %std dev fQ is mean fQ value from site times CV for closest u* at Oceano
% %         fQ_ust_bin_SE_all{i}(k) = fQ_std_pred/sqrt(ust_bin_N_all{i}(k)); %use this std dev to get SE
...
tau_bin_stdalt_all = cell(N_Sites,1); %compute alternative standard deviation of values of bin based on max of actual standard deviation versus expectation from mean uncertainty
...
sigma_Q_tau_bin_values_all = cell(N_Sites,1); %sigma_Q into tau bins
Q_tau_bin_avg_all = cell(N_Sites,1); %get average Q for tau bins
Q_tau_bin_sigmaavg_all = cell(N_Sites,1); %get uncertainty in average Q for tau bins
Q_tau_bin_std_all = cell(N_Sites,1); %get std Q for tau bins
Q_tau_bin_stdalt_all = cell(N_Sites,1); %compute alternative standard deviation of values of bin based on max of actual standard deviation versus expectation from mean uncertainty
Q_tau_bin_SE_all = cell(N_Sites,1); %get SE Q for tau bins
Q_tau_bin_sigmaQ_all = cell(N_Sites,1); %Q contribution to sigma_Q
Q_tau_bin_sigmatau_all = cell(N_Sites,1); %tau contribution to sigma_Q
Q_tau_bin_sigma_all = cell(N_Sites,1);  %total uncertainty in Q from Q and tau contributions

%get binned values for all sites
for i = 1:N_Sites
    
    %use only values above minimum tau (just set it to zero unless there is a strong reason otherwise)
    ind_analysis = find(tauRe_all{i}>tau_bin_lower);
    tau = tauRe_all{i}(ind_analysis);
    fQ = fQ_all{i}(ind_analysis);
    Q = Q_all{i}(ind_analysis);
    sigma_Q = sigma_Q_all{i}(ind_analysis);
    sigma_tau = sigma_tauRe_all{i}(ind_analysis);
    ...
    tau_bin_std_all{i} = tau_bin_SE.*sqrt(tau_bin_N); %std dev of tau in bin
    tau_bin_SE_all{i} = tau_bin_SE; %SE tau in bin
    N_tau_bins = length(tau_bin_values); %number of tau bins
    
    %get fQ, Q, and sigma_Q values in bins
    fQ_tau_bin_values_all{i} = cell(N_tau_bins,1); %initialize fQ's
    Q_tau_bin_values_all{i} = cell(N_tau_bins,1); %initialize Q's
    sigma_Q_tau_bin_values_all{i} = cell(N_tau_bins,1); %initialize sigma_Q's
    sigma_tau_bin_values_all{i} = cell(N_tau_bins,1); %initialize sigma_tau's
    for j = 1:N_tau_bins
        %get indices of values in bin
        ...
        sigma_Q_tau_bin_values_all{i}{j} = sigma_Q(bin_ind); %sigma_Q's
    ...
    %uncertainty in mean values for bins
    %tau_bin_sigmaavg_all{i} = tau_bin_avg_all{i}.*tau_relative_uncertainty./sqrt(tau_bin_N_all{i}); %uncertainty for tau in each bin is relative uncertainty divided by sqrt(n)
    tau_bin_sigmaavg_all{i} = zeros(N_tau_bins,1); %initialize list of uncertainties for mean tau in bin
    Q_tau_bin_sigmaavg_all{i} = zeros(N_tau_bins,1); %initialize list of uncertainties for mean Q in bin
    for j = 1:N_tau_bins
        [~, Q_tau_bin_sigmaavg] = MeanUncertainty(Q_tau_bin_values_all{i}{j},sigma_Q_tau_bin_values_all{i}{j}); %get uncertainty in avg Q for bin
        Q_tau_bin_sigmaavg_all{i}(j) = Q_tau_bin_sigmaavg;
...
%go through all bins, take sigmaQ (total flux uncertainty) as max of sigmaavg (uncertainty of mean) and SE (uncertainty of standard error)
for i = 1:N_Sites
    Q_tau_bin_sigmaQ_all{i} = max([Q_tau_bin_SE_all{i}, Q_tau_bin_sigmaavg_all{i}]')'; %total Q uncertainty (max of SE or uncertainty in mean)
    Q_tau_bin_stdalt_all{i} = max([Q_tau_bin_std_all{i}, Q_tau_bin_sigmaavg_all{i}.*sqrt(tau_bin_N_all{i})]')'; %estimate of Q standard deviation, either the actual std dev for bin or std dev estimated from uncertainty on mean for bin
    tau_bin_sigma_all{i} = max([tau_bin_SE_all{i}, tau_bin_sigmaavg_all{i}]')'; %total tau uncertainty (max of SE or uncertainty in mean)
    tau_bin_stdalt_all{i} = max([tau_bin_std_all{i}, tau_bin_sigmaavg_all{i}.*sqrt(tau_bin_N_all{i})]')'; %estimate of tau standard deviation, either the actual std dev for bin or std dev estimated from uncertainty on mean for bin
end
...
tau_CV_stdalt_Oceano = tau_bin_stdalt_all{i_Oceano}(ind_SE_Oceano)./(tau_bin_avg_all{i_Oceano}(ind_SE_Oceano)); %std/mean of tau values for these bins, using alternate standard dev estimate
...
Q_std_Oceano = Q_tau_bin_std_all{i_Oceano}(ind_SE_Oceano); %std of Q values for these bins
Q_stdalt_Oceano = Q_tau_bin_stdalt_all{i_Oceano}(ind_SE_Oceano); %std of Q values for these bins, using alternate standard dev estimate
Q_CV_std_Oceano = Q_std_Oceano./Q_tau_bin_avg_all{i_Oceano}(ind_SE_Oceano); %std/mean of Q values for these bins
Q_CV_stdalt_Oceano = Q_stdalt_Oceano./(Q_tau_bin_avg_all{i_Oceano}(ind_SE_Oceano)); %alternative std/mean of Q values for these bins
Q_CV_SE_Oceano = Q_tau_bin_SE_all{i_Oceano}(ind_SE_Oceano)./Q_tau_bin_avg_all{i_Oceano}(ind_SE_Oceano); %SE/mean of Q values for these bins
Q_CV_sigmaQ_Oceano = Q_tau_bin_sigmaQ_all{i_Oceano}(ind_SE_Oceano)./Q_tau_bin_avg_all{i_Oceano}(ind_SE_Oceano); %SE/mean of Q values for these bins
for i = 1:N_Sites
    ...
    for j = 1:N_ind
        ...
        tau_std_pred = tau_CV_stdalt_Oceano(ind_Oceano)*tau_bin_avg_all{i}(k); %predicted std dev tau is mean tau value from site times CV for closest tau at Oceano
        tau_sigma_RE = tau_std_pred/sqrt(tau_bin_N_all{i}(k)); %use this std dev to get SE
        tau_bin_sigma_all{i}(k) = max([tau_sigma_RE, tau_bin_sigma_all{i}(k)]); %reassign new sigma as max of current and tau_sigma_RE
        fQ_std_pred = fQ_CV_Oceano(ind_Oceano)*fQ_tau_bin_avg_all{i}(k); %std dev fQ is mean fQ value from site times CV for closest tau at Oceano
        fQ_tau_bin_SE_all{i}(k) = fQ_std_pred/sqrt(tau_bin_N_all{i}(k)); %use this std dev to get SE
        %Q_std_pred = Q_CV_stdalt_Oceano(ind_Oceano)*Q_tau_bin_avg_all{i}(k); %std dev Q is mean Q value from site times CV for closest tau at Oceano
        Q_std_pred = mean(Q_std_Oceano); %pred std dev of Q based on mean std dev of Q for Oceano points
        Q_sigmaQ_RE = Q_std_pred/sqrt(tau_bin_N_all{i}(k)); %use this std dev to estimate sigma_Q from relative error
        Q_tau_bin_sigmaQ_all{i}(k) = max([Q_sigmaQ_RE, Q_tau_bin_sigmaQ_all{i}(k)]); %reassign new sigma as max of current and Q_sigma_RE
    end
end

% %plot std dev for Oceano bins with sufficient fQ
% ind_transport_bins = find(fQ_tau_bin_avg_all{i_Oceano}(ind_SE_Oceano)>=fQ_Q_tau);
% figure; hold on;
% plot(tau_avg_Oceano(ind_transport_bins), Q_stdalt_Oceano(ind_transport_bins), Markers_field{i_Oceano},'MarkerSize',MarkerSize_field,'Color',Colors_field{i_Oceano});
% set(gca,'XMinorTick','On','YMinorTick','On','Box','On');
% xlabel('$$\tau$$ (Pa)','Interpreter','Latex');
% ylabel('$$s_{Q}$$ (g/m/s)','Interpreter','Latex');
% set(gca,'FontSize',PlotFont);
% set(gcf, 'PaperPosition',[0 0 8 6]);
% print([folder_Plots,'std_Q_tau.png'],'-dpng');
% 
% %plot coefficient of variation (std dev / mean) for Oceano bins with sufficient fQ
% ind_transport_bins = find(fQ_tau_bin_avg_all{i_Oceano}(ind_SE_Oceano)>=fQ_Q_tau);
% figure; hold on;
% plot(tau_avg_Oceano(ind_transport_bins), Q_CV_stdalt_Oceano(ind_transport_bins), Markers_field{i_Oceano},'MarkerSize',MarkerSize_field,'Color',Colors_field{i_Oceano});
% plot(tau_avg_Oceano(ind_transport_bins), Q_CV_sigmaQ_Oceano(ind_transport_bins), 'xb','MarkerSize',MarkerSize_field);
% set(gca,'XMinorTick','On','YMinorTick','On','Box','On');
% xlabel('$$\tau$$ (Pa)','Interpreter','Latex');
% ylabel('Coefficient of variation','Interpreter','Latex');
% h = legend('$$s_{Q}/\mu_{Q}$$','$$\sigma_{Q}/\mu_{Q}$$');
% set(h,'Interpreter','Latex');
% set(gca,'FontSize',PlotFont);
% set(gcf, 'PaperPosition',[0 0 8 6]);
% print([folder_Plots,'CV_Q_tau.png'],'-dpng');
...
%get values for nonlinear fit
tau_pred_nonlinearfit_all = cell(N_Sites,1); %tau for Q predictions for nonlinear fit
Q_pred_nonlinearfit_all = cell(N_Sites,1); %predicted Q from nonlinear fit
p_nonlinearfit_all = zeros(N_Sites,1); %p-value for nonlinear fit

%get values for describing Q uncertainty
Q_diff_all = cell(N_Sites,1); %get values for Q_diff_all = (Q - Q_pred)
Q_diff_ratio_all = cell(N_Sites,1); %get values for Q_diff_ratio_all = (Q - Q_pred)/Q
...
for i = 1:N_Sites
    ...
    sigma_Q_fit = Q_tau_bin_sigmaQ_all{i}(ind_fit);
    ...
    tau_pred_nonlinearfit_all{i} = linspace(tauit_nonlinearfit(i),max(tau_fit),50);
    ust_pred_nonlinear = sqrt(tau_pred_nonlinearfit_all{i}/rho_a);
    Q_pred_nonlinearfit_all{i} = C_nonlinearfit(i)*ust_pred_nonlinear.^n_nonlinearfit(i).*(tau_pred_nonlinearfit_all{i}-tauit_nonlinearfit(i));
    ust = sqrt(tau_bin_avg_all{i}/rho_a);
    
    %estimate tau contribution to Q uncertainty, then compute total Q uncertainty from this 
    tau_uncertainty_lower = tau_bin_avg_all{i} - tau_bin_sigma_all{i}; %values of tau for lower limit of tau uncertainty range
    tau_uncertainty_upper = tau_bin_avg_all{i} + tau_bin_sigma_all{i}; %values of tau for upper limit of tau uncertainty range
    ust_uncertainty_lower = sqrt(tau_uncertainty_lower/rho_a); %values of ust for lower limit of uncertainty range
    ust_uncertainty_upper = sqrt(tau_uncertainty_upper/rho_a); %values of ust for upper limit of tau uncertainty range
    Q_pred_tau_uncertainty_lower = C_nonlinearfit(i)*ust_uncertainty_lower.^n_nonlinearfit(i).*(tau_uncertainty_lower-tauit_nonlinearfit(i)); %values of Q for lower limit of tau uncertainty range
    Q_pred_tau_uncertainty_upper = C_nonlinearfit(i)*ust_uncertainty_upper.^n_nonlinearfit(i).*(tau_uncertainty_upper-tauit_nonlinearfit(i)); %values of Q for upper limit of tau uncertainty range
    Q_tau_bin_sigmatau_all{i} = (Q_pred_tau_uncertainty_upper-Q_pred_tau_uncertainty_lower)/2;
    %Q_tau_bin_sigmatau_all{i} = C_nonlinearfit(i)*ust.^n_nonlinearfit(i).*tau_bin_sigma_all{i}; %predicted tau contribution to sigma_Q for nonlinear fit parameters
    Q_tau_bin_sigma_all{i} = sqrt(Q_tau_bin_sigmaQ_all{i}.^2+Q_tau_bin_sigmatau_all{i}.^2); %predicted total Q uncertainty for nonlinear fit parameters
    p_nonlinearfit_all(i) = chi2cdf(Chi2_nonlinearfit(i),length(Chi2_contributions_nonlinearfit{i})-3);
    
    %determine Q_diff_all (Q - Q_pred)
    Q_diff_all{i} = (Q_tau_bin_avg_all{i}-C_nonlinearfit(i)*ust.^n_nonlinearfit(i).*(tau_bin_avg_all{i}-tauit_nonlinearfit(i)));
    
    %determine Q_diff_ratio_all (Q - Q_pred)/Q
    Q_diff_ratio_all{i} = (Q_tau_bin_avg_all{i}-C_nonlinearfit(i)*ust.^n_nonlinearfit(i).*(tau_bin_avg_all{i}-tauit_nonlinearfit(i)))./Q_tau_bin_avg_all{i};
end

%% PLOT UNCERTAINTY VALUES
for i = 1:N_Sites
    ind_plot = find(tau_bin_avg_all{i}>tauit_fit_all(i)+0.02);
    N_subplot = 11;
    figure(i);
    
    subplot(N_subplot,1,1:2); cla; hold on;
    plot(tau_bin_avg_all{i}(ind_plot),Q_tau_bin_sigma_all{i}(ind_plot),'*','MarkerSize',MarkerSize_field,'Color',Colors_field{1});
    plot(tau_bin_avg_all{i}(ind_plot),Q_tau_bin_sigmatau_all{i}(ind_plot),Markers_field{2},'MarkerSize',MarkerSize_field/2,'Color',Colors_field{2});
    plot(tau_bin_avg_all{i}(ind_plot),Q_tau_bin_sigmaQ_all{i}(ind_plot),Markers_field{3},'MarkerSize',MarkerSize_field/2,'Color',Colors_field{3});
    title(Sites{i});
    set(gca,'XMinorTick','On','YMinorTick','On','Box','On');
    ylabel('\textbf{$$\sigma_{Q}$$ (g/m/s)}','Interpreter','Latex');
    legend('total uncertainty','uncertainty from \tau','uncertainty from Q measurement','Location','NorthWest')
    
    subplot(N_subplot,1,3); cla; hold on;
    plot(tau_bin_avg_all{i}(ind_plot),Q_tau_bin_sigma_all{i}(ind_plot)./Q_tau_bin_avg_all{i}(ind_plot),'*','MarkerSize',MarkerSize_field,'Color',Colors_field{1});
    plot(tau_bin_avg_all{i}(ind_plot),Q_tau_bin_sigmatau_all{i}(ind_plot)./Q_tau_bin_avg_all{i}(ind_plot),Markers_field{2},'MarkerSize',MarkerSize_field/2,'Color',Colors_field{2});
    plot(tau_bin_avg_all{i}(ind_plot),Q_tau_bin_sigmaQ_all{i}(ind_plot)./Q_tau_bin_avg_all{i}(ind_plot),Markers_field{3},'MarkerSize',MarkerSize_field/2,'Color',Colors_field{3});
    set(gca,'XMinorTick','On','YMinorTick','On','Box','On');
    ylabel('\textbf{$$\sigma_{Q} / Q$$}','Interpreter','Latex');

    subplot(N_subplot,1,4:5); cla; hold on;
    plot(tau_bin_avg_all{i}(ind_plot),Q_tau_bin_sigmaQ_all{i}(ind_plot),'*','MarkerSize',MarkerSize_field,'Color',Colors_field{1});
    plot(tau_bin_avg_all{i}(ind_plot),Q_tau_bin_SE_all{i}(ind_plot),Markers_field{2},'MarkerSize',MarkerSize_field/2,'Color',Colors_field{2});
    plot(tau_bin_avg_all{i}(ind_plot),Q_tau_bin_sigmaavg_all{i}(ind_plot),Markers_field{3},'MarkerSize',MarkerSize_field/2,'Color',Colors_field{3});
    set(gca,'XMinorTick','On','YMinorTick','On','Box','On');
    ylabel('\textbf{$$\sigma_{Q,m}$$ (g/m/s)}','Interpreter','Latex');
    legend('total uncertainty for Q measurement','bin SE for Q measurement','bin mean uncertainty for Q measurement','Location','NorthWest')
    
    subplot(N_subplot,1,6); cla; hold on;
    plot(tau_bin_avg_all{i}(ind_plot),tau_bin_N_all{i}(ind_plot),Markers_field{1},'MarkerSize',MarkerSize_field,'Color',Colors_field{1});
    plot(xlim,[bin_N_min bin_N_min]);
    set(gca,'XMinorTick','On','YMinorTick','On','Box','On');
    ylabel('N in bin');
    
    subplot(N_subplot,1,7:8); cla; hold on;
    plot(tau_bin_avg_all{i}(ind_plot),tau_bin_sigma_all{i}(ind_plot),'*','MarkerSize',MarkerSize_field,'Color',Colors_field{1});
    plot(tau_bin_avg_all{i}(ind_plot),tau_bin_SE_all{i}(ind_plot),Markers_field{2},'MarkerSize',MarkerSize_field/2,'Color',Colors_field{2});
    plot(tau_bin_avg_all{i}(ind_plot),tau_bin_sigmaavg_all{i}(ind_plot),Markers_field{3},'MarkerSize',MarkerSize_field/2,'Color',Colors_field{3});
    set(gca,'XMinorTick','On','YMinorTick','On','Box','On');
    ylabel('\textbf{$$\sigma_{\tau}$$ (Pa)}','Interpreter','Latex');
    legend('total uncertainty for \tau','bin SE for \tau','bin mean uncertainty for \tau','Location','NorthWest')
    
    subplot(N_subplot,1,9); cla; hold on;
    plot(tau_bin_avg_all{i}(ind_plot),tau_bin_sigma_all{i}(ind_plot)./tau_bin_avg_all{i}(ind_plot),'*','MarkerSize',MarkerSize_field,'Color',Colors_field{1});
    plot(tau_bin_avg_all{i}(ind_plot),tau_bin_SE_all{i}(ind_plot)./tau_bin_avg_all{i}(ind_plot),Markers_field{2},'MarkerSize',MarkerSize_field/2,'Color',Colors_field{2});
    plot(tau_bin_avg_all{i}(ind_plot),tau_bin_sigmaavg_all{i}(ind_plot)./tau_bin_avg_all{i}(ind_plot),Markers_field{3},'MarkerSize',MarkerSize_field/2,'Color',Colors_field{3});

    
    %plot(tau_bin_avg_all{i}(ind_plot),tau_bin_sigma_all{i}(ind_plot)./tau_bin_avg_all{i}(ind_plot),Markers_field{1},'MarkerSize',MarkerSize_field/2,'Color',Colors_field{1});
    set(gca,'XMinorTick','On','YMinorTick','On','Box','On');
    xlabel('\textbf{$$\tau$$ (Pa)}','Interpreter','Latex');
    ylabel('\textbf{$$\sigma_{\tau} / \tau$$}','Interpreter','Latex');
    
    subplot(N_subplot,1,10); cla; hold on;
    plot(tau_nonlinearfit{i},Chi2_contributions_nonlinearfit{i},Markers_field{1},'MarkerSize',MarkerSize_field,'Color',Colors_field{1});
    set(gca,'XMinorTick','On','YMinorTick','On','Box','On');
    xlabel('\textbf{$$\tau$$ (Pa)}','Interpreter','Latex');
    ylabel('\textbf{$$\chi^{2}_{i}$$}','Interpreter','Latex');
    
    subplot(N_subplot,1,11); cla; hold on;
    plot(tau_nonlinearfit{i},Chi2_contributions_nonlinearfit{i}/Chi2_nonlinearfit(i),Markers_field{1},'MarkerSize',MarkerSize_field,'Color',Colors_field{1});
    set(gca,'XMinorTick','On','YMinorTick','On','Box','On');
    xlabel('\textbf{$$\tau$$ (Pa)}','Interpreter','Latex');
    ylabel('\textbf{$$\chi^{2}_{i}/\chi^{2}$$}','Interpreter','Latex');
    
    set(gcf, 'PaperPosition',[0 0 6 16]);
    print([folder_Plots,'uncertainty_',Sites{i},'.png'],'-dpng');
end
...
x = linspace(0,10,100);
plot(x,chi2cdf(x,3),'k');
plot(Chi2_nonlinearfit(1)*ones(2,1),[0 1],'Color',Colors_field{1});
plot(Chi2_nonlinearfit(2)*ones(2,1),[0 1],'Color',Colors_field{2});
xlabel('\chi^2');
ylabel('CDF');
legend('\chi^2 CDF','Jericoacoara','Rancho Guadalupe','Location','SouthEast');
set(gca,'XMinorTick','On','YMinorTick','On','Box','On');

subplot(2,1,2); hold on;
x = linspace(0,30,100);
plot(x,chi2cdf(x,13),'k');
plot(Chi2_nonlinearfit(3)*ones(2,1),[0 1],'Color',Colors_field{3});
xlabel('\chi^2');
ylabel('CDF');
legend('\chi^2 CDF','Oceano','Location','SouthEast');
set(gca,'XMinorTick','On','YMinorTick','On','Box','On');

print([folder_Plots,'chi2_CDF.png'],'-dpng');
...
for i = 1:N_Sites
    ind_plot = find(fQ_tau_bin_avg_all{i}>fQ_Q_tau);
    errorbar(tau_bin_avg_all{i}(ind_plot),Q_tau_bin_avg_all{i}(ind_plot),Q_tau_bin_sigma_all{i}(ind_plot),Markers_field{i},'MarkerSize',MarkerSize_field,'Color',Colors_field{i}); % include uncertainties from tau
    %errorbar(tau_bin_avg_all{i}(ind_plot),Q_tau_bin_avg_all{i}(ind_plot),Q_tau_bin_sigmaQ_all{i}(ind_plot),Markers_field{i},'MarkerSize',MarkerSize_field,'Color',Colors_field{i}); % only show Q uncertainties
    plot(tau_fit_all{i},Q_pred_all{i},'Color',Colors_field{i}); %plot fit for these values
    legend_items{3*i-2} = [Sites{i}, ', df = ',int2str(length(ind_plot)-3)]; %add to list of legend items
    legend_items{3*i-1} = 'linear fit'; %add to list of legend items
    legend_items{3*i} = ['nonlinear, n=[',...
        num2str(n_range_confidence_nonlinearfit(i,1),'%.2f'),', ',...
        num2str(n_range_confidence_nonlinearfit(i,2),'%.2f'),'], \chi^2 = ',...
        num2str(Chi2_nonlinearfit(i),'%.1f'),', p = ',...
        num2str(p_nonlinearfit_all(i),'%.2f')]; %add to list of legend items
    plot(tau_pred_nonlinearfit_all{i},Q_pred_nonlinearfit_all{i},'--','Color',Colors_field{i}); %plot nonlinear fit
end
xlim([0.05 0.35]);
ylim([0 50]);
set(gca,'XMinorTick','On','YMinorTick','On','Box','On');
...
%% PLOT Q_DIFF = Q-Q_PRED VERSUS TAU
figure; hold on;
legend_items = cell(N_Sites,1);
for i = 1:N_Sites
    ind_plot = find(fQ_tau_bin_avg_all{i}>fQ_Q_tau);
    errorbar(tau_bin_avg_all{i}(ind_plot),Q_diff_all{i}(ind_plot),Q_tau_bin_sigma_all{i}(ind_plot),Markers_field{i},'MarkerSize',MarkerSize_field,'Color',Colors_field{i}); % include uncertainties from tau
    legend_items{i} = [Sites{i}, ', df = ',int2str(length(ind_plot)-3),...
        ', n = [', num2str(n_range_confidence_nonlinearfit(i,1),'%.2f'),', ',...
        num2str(n_range_confidence_nonlinearfit(i,2),'%.2f'),'], \chi^2 = ',...
        num2str(Chi2_nonlinearfit(i),'%.1f'),', p = ',...
        num2str(p_nonlinearfit_all(i),'%.2f')]; %add to list of legend items
end
plot([0.05 0.35],[0 0]); %plot zero line
xlim([0.05 0.35]);
ylim([-10 10]);
set(gca,'XMinorTick','On','YMinorTick','On','Box','On');
xlabel('\textbf{$$\tau$$ (Pa)}','Interpreter','Latex');
ylabel('\textbf{$$Q - Q_{pred}$$ (g/m/s)}','Interpreter','Latex');
legend(legend_items,'Location','NorthWest');
set(gca,'FontSize',PlotFont);
set(gcf, 'PaperPosition',[0 0 8 6]);
print([folder_Plots,'Qdiff_tau.png'],'-dpng');


%% PLOT Q_DIFF_RATIO = Q-Q_PRED/Q VERSUS TAU
figure;
legend_items = cell(N_Sites,1);

subplot(2,1,1); hold on; %plot all
for i = 1:N_Sites
    ind_plot = find(fQ_tau_bin_avg_all{i}>fQ_Q_tau);
    errorbar(tau_bin_avg_all{i}(ind_plot),Q_diff_ratio_all{i}(ind_plot),Q_tau_bin_sigma_all{i}(ind_plot)./Q_tau_bin_avg_all{i}(ind_plot),Markers_field{i},'MarkerSize',MarkerSize_field,'Color',Colors_field{i}); % include uncertainties from tau
    legend_items{i} = [Sites{i}, ', df = ',int2str(length(ind_plot)-3),...
        ', n = [', num2str(n_range_confidence_nonlinearfit(i,1),'%.2f'),', ',...
        num2str(n_range_confidence_nonlinearfit(i,2),'%.2f'),'], \chi^2 = ',...
        num2str(Chi2_nonlinearfit(i),'%.1f'),', p = ',...
...
subplot(2,1,2); hold on; %plot again, but zoomed in
for i = 1:N_Sites
    ind_plot = find(fQ_tau_bin_avg_all{i}>fQ_Q_tau);
    errorbar(tau_bin_avg_all{i}(ind_plot),Q_diff_ratio_all{i}(ind_plot),Q_tau_bin_sigma_all{i}(ind_plot)./Q_tau_bin_avg_all{i}(ind_plot),Markers_field{i},'MarkerSize',MarkerSize_field,'Color',Colors_field{i}); % include uncertainties from tau
    legend_items{i} = [Sites{i}, ', df = ',int2str(length(ind_plot)-3),...
...
print([folder_Plots,'Qdiffratio_tau.png'],'-dpng');
...
% Q_CV_std_Oceano = Q_tauex_bin_std_all{i_Oceano}(ind_SE_Oceano)./Q_tauex_bin_avg_all{i_Oceano}(ind_SE_Oceano); %std/mean of Q values for these bins
% for i = 1:N_Sites
...
%         fQ_std_pred = fQ_CV_Oceano(ind_Oceano)*fQ_tauex_bin_avg_all{i}(k); %std dev fQ is mean fQ value from site times CV for closest tauex at Oceano
%         fQ_tauex_bin_SE_all{i}(k) = fQ_std_pred/sqrt(tauex_bin_N_all{i}(k)); %use this std dev to get SE
%         Q_std_pred = Q_CV_std_Oceano(ind_Oceano)*Q_tauex_bin_avg_all{i}(k); %std dev Q is mean Q value from site times CV for closest tauex at Oceano
%         Q_tauex_bin_SE_all{i}(k) = Q_std_pred/sqrt(tauex_bin_N_all{i}(k)); %use this std dev to get SE
%     end
% end
% 
...
% 
% %% PLOT Q VERSUS TRANSPORT FREQUENCY
% figure; hold on;
% for i = 1:N_Sites
%     subplot(1,N_Sites,i);
...
% 
% %% PLOT TRANSPORT FREQUENCY VERSUS SHEAR STRESS
% figure; hold on;
% for i = 1:N_Sites
%     errorbar(tau_bin_avg_all{i},fQ_tau_bin_avg_all{i},fQ_tau_bin_SE_all{i},Markers_field{i},'LineWidth',LineWidth_field,'MarkerSize',MarkerSize_field,'Color',Colors_field{i});
% end
% %set(gca,'yscale','log');
% xlim([0 0.35]);
% ylim([0 1]);
