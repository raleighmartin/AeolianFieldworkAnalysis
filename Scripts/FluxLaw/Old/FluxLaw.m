...
%% PLOT NORMALIZED Q/(ustit/g) VERSUS TAUEX BINNED DATA
figure(4); clf; hold on;

%get values
tauex_Qtaufit_all = cell(N_Sites,1);
tauex_sigma_Qtaufit_all = cell(N_Sites,1);
Qnorm_Qtaufit_all = cell(N_Sites,1);
Qnorm_sigma_Qtaufit_all = cell(N_Sites,1);
for i = 1:N_Sites
    tauex_Qtaufit_all{i} = tau_Qtaufit_all{i} - tauit_linearfit_all(i);
    tauex_sigma_Qtaufit_all{i} = sqrt(tau_sigma_Qtaufit_all{i}.^2+tauit_sigma_linearfit_all(i).^2);
    Qnorm_Qtaufit_all{i} = 0.001*Q_Qtaufit_all{i}.*g./ustit_linearfit_all(i);
    Qnorm_sigma_Qtaufit_all{i} = Qnorm_Qtaufit_all{i}.*sqrt((Q_sigma_Qtaufit_all{i}./Q_Qtaufit_all{i}).^2+(tauit_sigma_linearfit_all(i)./tauit_linearfit_all(i)).^2);
end
    
%plot binned values
for i = 1:N_Sites
    plot(tauex_Qtaufit_all{i},Qnorm_Qtaufit_all{i},Markers_Field{i},'Color',Colors_Field{i},'MarkerSize',MarkerSize_Field);
end

%plot error bars
for i = 1:N_Sites
    for j = 1:length(tauex_Qtaufit_all{i})
        plot(ones(2,1)*tauex_Qtaufit_all{i}(j),Qnorm_Qtaufit_all{i}(j)+[-1 1]*Qnorm_sigma_Qtaufit_all{i}(j),'Color',Colors_Field{i},'LineWidth',LineWidth_Field); %y error
        plot(tauex_Qtaufit_all{i}(j)+[1 -1]*tauex_sigma_Qtaufit_all{i}(j),ones(2,1)*Qnorm_Qtaufit_all{i}(j),'Color',Colors_Field{i},'LineWidth',LineWidth_Field); %x error
    end
end
...
ylim([0 3]);
set(gca,'XMinorTick','On','YMinorTick','On','Box','On');
xlabel('\textbf{Excess shear stress, $$\tau - \tau_{it}$$ (Pa)}','Interpreter','Latex');
ylabel('\textbf{Normalized saltation flux, $$\frac{Q_s}{g/u_{*,it}}$$ (Pa)}','Interpreter','Latex');
legend(SiteNames,'Location','NorthWest');
set(gca,'FontSize',PlotFont);
...
print([folder_Plots,'Q_tauex_norm.png'],'-dpng');

%plot fit values
tauex_fit = [0 0.35];
for i = 1:N_Sites
    plot(tauex_fit, (Qtauexratio_bar_all(i)*tauex_fit)/(1000*ustit_linearfit_all(i)/g),'Color',Colors_Field{i});
end
print([folder_Plots,'Q_tauex_norm_fit.png'],'-dpng');

plot(tauex_fit, 5.9*tauex_fit,'k')
plot(tauex_fit, 6.9*tauex_fit,'k')
print([folder_Plots,'Q_tauex_norm_fitrange.png'],'-dpng');
...
for i = 1:N_Sites
    ...
    %plot data
    plot(ust_plot,zq_plot,Markers_Field{i},'Color',Colors_Field{i},'MarkerSize',MarkerSize_Field);

    %plot error bars
    for j = 1:length(ust_plot)
        plot(ones(2,1)*ust_plot(j),zq_plot(j)+[-1 1]*sigma_zq_plot(j),'Color',Colors_Field{i},'LineWidth',LineWidth_Field); %y-error
        plot(ust_plot(j)+[-1 1]*sigma_ust_plot(j),ones(2,1)*zq_plot(j),'Color',Colors_Field{i},'LineWidth',LineWidth_Field); %x-error
    end
...
%ylabel('\textbf{Saltation mass flux, $$Q$$ (gm$$^{-1}$$s$$^{-1}$$)}','Interpreter','Latex');
ylabel('\textbf{Saltation flux, $$Q_{s}$$ (gm$$^{-1}$$s$$^{-1}$$)}','Interpreter','Latex');
