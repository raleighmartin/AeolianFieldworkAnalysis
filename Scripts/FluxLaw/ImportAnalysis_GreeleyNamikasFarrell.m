...
folder_SaveData = '../../AnalysisData/FluxLaw/'; %folder for storing outputs of this analysis
...
for i = 1:N_Greeley96
    sigma_q_Greeley96{i} = zeros(size(q_Greeley96{i}));
    sigma_z_Greeley96{i} = zeros(size(z_Greeley96{i}));
end
...
for i = 1:N_Namikas03
    sigma_q_Namikas03{i} = zeros(size(q_Namikas03{i}));
    sigma_z_Namikas03{i} = zeros(size(z_Namikas03{i}));
end

for i=1:N_Namikas03
    [q0,zq,Q,sigma_q0,sigma_zq,sigma_Q] = qz_profilefit_exponential(q_Namikas03{i}, z_Namikas03{i}, sigma_q_Namikas03{i}, sigma_z_Namikas03{i}, zq_Greeley96_assumed); %fit will assume equal uncertainties
...
% Assign uncertainties - initialize
sigma_q_Farrell12 = cell(N_Farrell12,1);
sigma_z_Farrell12 = cell(N_Farrell12,1);
...
for i = 1:N_Farrell12
    ...
    % Assign uncertainties
    sigma_q_Farrell12{i} = zeros(size(q_Farrell12{i}));
    sigma_z_Farrell12{i} = zeros(size(z_bottom_profile));
    ...
    [z_profile,q0,zq,Q,~,sigma_zq,sigma_Q,~,~,~,~,z_profile_geomean,q0_geomean,zq_geomean,Q_geomean] = ...
        BSNE_profilefit_exponential(q_Farrell12{i}, z_bottom_profile, H_profile, sigma_q_Farrell12{i}, sigma_z_Farrell12{i}, zq_Farrell12_assumed);
%     [z_profile,q0,zq,Q,~,sigma_zq,sigma_Q,~,~,~,~,z_profile_geomean,q0_geomean,zq_geomean,Q_geomean] = ...
%         BSNE_profilefit(q_Farrell12{i}, z_bottom_profile, H_profile);
