...
median_Qfit_calc_analysis = cell(N_Sites,1); %median fitting flux
median_Qsum_calc_analysis = cell(N_Sites,1); %median summation flux
median_Qsum_all_analysis = cell(N_Sites,1); %median summation flux
median_sigma_Qfit_calc_analysis = cell(N_Sites,1); %median fitting flux uncertainty
median_sigma_Qsum_calc_analysis = cell(N_Sites,1); %median summation flux uncertainty
median_sigma_Qsum_all_analysis = cell(N_Sites,1); %median summation flux uncertainty
mean_Qfit_calc_analysis = cell(N_Sites,1); %mean fitting flux
mean_Qsum_calc_analysis = cell(N_Sites,1); %mean summation flux
mean_Qsum_all_analysis = cell(N_Sites,1); %mean summation flux
mean_sigma_Qfit_calc_analysis = cell(N_Sites,1); %mean fitting flux uncertainty
mean_relative_sigma_Qfit_calc_analysis = cell(N_Sites,1); %mean relative fitting flux uncertainty
mean_sigma_Qsum_calc_analysis = cell(N_Sites,1); %mean summation flux uncertainty
mean_sigma_Qsum_all_analysis = cell(N_Sites,1); %mean summation flux uncertainty
mean_sigma_Qsum_positive_analysis = cell(N_Sites,1); %mean summation flux uncertainty
mean_relative_sigma_Qsum_positive_analysis = cell(N_Sites,1); %mean relative summation flux uncertainty
SD_Qfit_calc_analysis = cell(N_Sites,1); %SD on Qfit calculation
SD_Qsum_calc_analysis = cell(N_Sites,1); %SD on Qsum calculation
SD_Qsum_all_analysis = cell(N_Sites,1); %SD on Qsum calculation
SE_Qfit_calc_analysis = cell(N_Sites,1); %SE on Qfit calculation
SE_Qsum_calc_analysis = cell(N_Sites,1); %SE on Qsum calculation
SE_Qsum_all_analysis = cell(N_Sites,1); %SE on Qsum calculation

%extra calcs
Qfit_real_analysis = cell(N_Sites,1); %total fluxes by fitting
Qsum_all_analysis = cell(N_Sites,1); %total fluxes by summation
Qsum_real_analysis = cell(N_Sites,1); %total fluxes by summation
Qsum_positive_analysis = cell(N_Sites,1); %get all total fluxes by summation
Qrel_real_analysis = cell(N_Sites,1); %relative total fluxes
median_Qfit_real_analysis = cell(N_Sites,1); %median fitting flux
median_Qsum_real_analysis = cell(N_Sites,1); %median summation flux
median_Qsum_positive_analysis = cell(N_Sites,1); %median summation flux
median_sigma_Qfit_real_analysis = cell(N_Sites,1); %median fitting flux uncertainty
median_sigma_Qsum_real_analysis = cell(N_Sites,1); %median summation flux uncertainty
median_sigma_Qsum_positive_analysis = cell(N_Sites,1); %median summation flux uncertainty
mean_Qfit_real_analysis = cell(N_Sites,1); %mean fitting flux
mean_Qsum_real_analysis = cell(N_Sites,1); %mean summation flux
mean_Qsum_all_analysis = cell(N_Sites,1); %mean summation flux
mean_sigma_Qfit_real_analysis = cell(N_Sites,1); %mean fitting flux uncertainty
mean_sigma_Qsum_real_analysis = cell(N_Sites,1); %mean summation flux uncertainty
mean_sigma_Qsum_all_analysis = cell(N_Sites,1); %mean summation flux uncertainty
...
for i = 1:N_Sites
    ...
    %compute median and mean values
    median_Qfit_calc_analysis{i} = zeros(N_T_subwindow,1); %median fitting flux
    median_Qsum_calc_analysis{i} = zeros(N_T_subwindow,1); %median summation flux
    median_Qsum_all_analysis{i} = zeros(N_T_subwindow,1); %median summation flux
    median_sigma_Qfit_calc_analysis{i} = zeros(N_T_subwindow,1); %median fitting flux uncertainty
    median_sigma_Qsum_calc_analysis{i} = zeros(N_T_subwindow,1); %median summation flux uncertainty
    median_sigma_Qsum_all_analysis{i} = zeros(N_T_subwindow,1); %median summation flux uncertainty
    mean_Qfit_calc_analysis{i} = zeros(N_T_subwindow,1); %mean fitting flux
    mean_Qsum_calc_analysis{i} = zeros(N_T_subwindow,1); %mean summation flux
    mean_Qsum_all_analysis{i} = zeros(N_T_subwindow,1); %mean summation flux
    mean_sigma_Qfit_calc_analysis{i} = zeros(N_T_subwindow,1); %mean fitting flux uncertainty
    mean_relative_sigma_Qfit_calc_analysis{i} = zeros(N_T_subwindow,1); %mean relative fitting flux uncertainty
    mean_sigma_Qsum_calc_analysis{i} = zeros(N_T_subwindow,1); %mean summation flux uncertainty
    mean_sigma_Qsum_all_analysis{i} = zeros(N_T_subwindow,1); %mean summation flux uncertainty
    mean_sigma_Qsum_positive_analysis{i} = zeros(N_T_subwindow,1); %mean summation flux uncertainty
    mean_relative_sigma_Qsum_positive_analysis{i} = zeros(N_T_subwindow,1); %mean relative summation flux uncertainty
    SD_Qfit_calc_analysis{i} = zeros(N_T_subwindow,1); %SD on Qfit calculation
    SD_Qsum_calc_analysis{i} = zeros(N_T_subwindow,1); %SD on Qsum calculation
    SD_Qsum_all_analysis{i} = zeros(N_T_subwindow,1); %SD on Qsum calculation
    SE_Qfit_calc_analysis{i} = zeros(N_T_subwindow,1); %SE on Qfit calculation
    SE_Qsum_calc_analysis{i} = zeros(N_T_subwindow,1); %SE on Qsum calculation
    SE_Qsum_all_analysis{i} = zeros(N_T_subwindow,1); %SE on Qsum calculation
    
    %extra calcs
    Qfit_real_analysis{i} = cell(N_T_subwindow,1);
    Qsum_all_analysis{i} = cell(N_T_subwindow,1);
    Qsum_real_analysis{i} = cell(N_T_subwindow,1);
    Qrel_real_analysis{i} = cell(N_T_subwindow,1);
    median_Qfit_real_analysis{i} = zeros(N_T_subwindow,1); %median fitting flux
    median_Qsum_real_analysis{i} = zeros(N_T_subwindow,1); %median summation flux
    median_Qsum_positive_analysis{i} = zeros(N_T_subwindow,1); %median summation flux
    median_sigma_Qfit_real_analysis{i} = zeros(N_T_subwindow,1); %median fitting flux uncertainty
    median_sigma_Qsum_real_analysis{i} = zeros(N_T_subwindow,1); %median summation flux uncertainty
    median_sigma_Qsum_positive_analysis{i} = zeros(N_T_subwindow,1); %median summation flux uncertainty
    mean_Qfit_real_analysis{i} = zeros(N_T_subwindow,1); %mean fitting flux
    mean_Qsum_real_analysis{i} = zeros(N_T_subwindow,1); %mean summation flux
    mean_Qsum_all_analysis{i} = zeros(N_T_subwindow,1); %mean summation flux
    mean_sigma_Qfit_real_analysis{i} = zeros(N_T_subwindow,1); %mean fitting flux uncertainty
    mean_sigma_Qsum_real_analysis{i} = zeros(N_T_subwindow,1); %mean summation flux uncertainty
    mean_sigma_Qsum_all_analysis{i} = zeros(N_T_subwindow,1); %mean summation flux uncertainty
    ...
    for m = 1:N_T_subwindow
        %get values
        Qfit_all = Qfit_subwindow{i}{m}; %get Qfit
        Qsum_all = Qsum_subwindow{i}{m}; %get Qsum
        sigma_Qfit_all = sigma_Qfit_subwindow{i}{m}; %get sigma Qfit
        sigma_Qsum_all = sigma_Qsum_subwindow{i}{m}; %get sigma Qsum
        
        %make some NaN's into 0's 
        ind_0 = find(Qsum_all==0);
        Qfit_all(ind_0) = 0;
        sigma_Qfit_all(ind_0) = 0;
        sigma_Qsum_all(ind_0) = 0;
        
        %get subsets of values
        ind_calc = union(ind_real{i}{m},ind_0); %indices of defined fits or Q = 0
        %ind_calc = union(intersect(ind_real{i}{m},ind_positive_Qsum{i}{m}),ind_0); %indices of defined fits or Q = 0
        Qfit_calc = Qfit_all(ind_calc);
        Qsum_calc = Qsum_all(ind_calc);
        sigma_Qfit_calc = sigma_Qfit_all(ind_calc);
        sigma_Qsum_calc = sigma_Qsum_all(ind_calc);
        
%         ind_real_Qsum = union(ind_positive_Qsum{i}{m},ind_0);
%         Qsum_real = Qsum_all(ind_real_Qsum);
%         sigma_Qsum_real = sigma_Qsum_all(ind_real_Qsum);
              
        %compute median and mean values
        median_Qfit_calc_analysis{i}(m) = median(Qfit_calc); %median fitting flux
        median_Qsum_calc_analysis{i}(m) = median(Qsum_calc(~isnan(Qsum_calc))); %median summation flux
        median_Qsum_all_analysis{i}(m) = median(Qsum_all(~isnan(Qsum_all))); %median summation flux
        median_sigma_Qfit_calc_analysis{i}(m) = median(sigma_Qfit_calc); %median fitting flux uncertainty
        median_sigma_Qsum_calc_analysis{i}(m) = median(sigma_Qsum_calc(~isnan(sigma_Qsum_calc))); %median summation flux uncertainty
        median_sigma_Qsum_all_analysis{i}(m) = median(sigma_Qsum_all(~isnan(sigma_Qsum_all))); %median summation flux uncertainty
        mean_Qfit_calc_analysis{i}(m) = mean(Qfit_calc); %mean fitting flux
        mean_Qsum_calc_analysis{i}(m) = mean(Qsum_calc(~isnan(Qsum_calc))); %mean summation flux
        mean_Qsum_all_analysis{i}(m) = mean(Qsum_all(~isnan(Qsum_all))); %mean summation flux
        mean_sigma_Qfit_calc_analysis{i}(m) = mean(sigma_Qfit_calc); %mean fitting flux uncertainty
        mean_relative_sigma_Qfit_calc_analysis{i}(m) = mean(sigma_Qfit_calc(Qfit_calc>0)./Qfit_calc(Qfit_calc>0)); %mean fitting flux uncertainty
        mean_sigma_Qsum_calc_analysis{i}(m) = mean(sigma_Qsum_calc(~isnan(sigma_Qsum_calc))); %mean summation flux uncertainty
        mean_sigma_Qsum_all_analysis{i}(m) = mean(sigma_Qsum_all(~isnan(sigma_Qsum_all))); %mean summation flux uncertainty
        mean_sigma_Qsum_positive_analysis{i}(m) = mean(sigma_Qsum_all(sigma_Qsum_all>0)); %mean summation flux uncertainty
        mean_relative_sigma_Qsum_positive_analysis{i}(m) = mean(sigma_Qsum_all(Qsum_all>0&sigma_Qsum_all>0)./Qsum_all(Qsum_all>0&sigma_Qsum_all>0)); %mean summation flux uncertainty
        SD_Qfit_calc_analysis{i}(m) = std(Qfit_calc); %mean fitting flux
        SD_Qsum_calc_analysis{i}(m) = std(Qsum_calc(~isnan(Qsum_calc))); %mean summation flux
        SD_Qsum_all_analysis{i}(m) = std(Qsum_all(~isnan(Qsum_all))); %mean summation flux
        SE_Qfit_calc_analysis{i}(m) = std(Qfit_calc)/sqrt(length(Qfit_calc)); %mean fitting flux
        SE_Qsum_calc_analysis{i}(m) = std(Qsum_calc(~isnan(Qsum_calc)))/sqrt(length(~isnan(Qsum_calc))); %mean summation flux
        SE_Qsum_all_analysis{i}(m) = std(Qsum_all(~isnan(Qsum_all)))/sqrt(length(~isnan(Qsum_all))); %mean summation flux
        
        %extra calcs
        Qfit_real = Qfit_all(ind_real{i}{m}); %get Qfit
        Qsum_real = Qsum_all(ind_real{i}{m}); %get Qsum
        Qsum_positive = Qsum_all(ind_positive_Qsum{i}{m}); %get Qsum
        sigma_Qsum_all(isnan(sigma_Qsum_all))=0; %set NaN's to 0's
        sigma_Qfit_real = sigma_Qsum_all(ind_real{i}{m}); %get sigma Qfit
        sigma_Qsum_real = sigma_Qsum_all(ind_real{i}{m}); %get sigma Qsum
        sigma_Qsum_positive = sigma_Qsum_all(ind_positive_Qsum{i}{m}); %get sigma Qsum
        Qrel_real = (Qfit_real-Qsum_real)./Qsum_real; %compute Qrel
        Qfit_real_analysis{i}{m} = Qfit_real; %add Qfit to list
        Qsum_real_analysis{i}{m} = Qsum_real; %add Qsum to list
        Qsum_all_analysis{i}{m} = Qsum_all; %add Qsum to list
        Qrel_real_analysis{i}{m} = Qrel_real; %add these to list
%         median_Qfit_real_analysis{i}(m) = median(Qfit_real); %median fitting flux
%         median_Qsum_real_analysis{i}(m) = median(Qsum_real); %median summation flux
%         median_Qsum_positive_analysis{i}(m) = median(Qsum_positive); %median summation flux
%         median_sigma_Qfit_real_analysis{i}(m) = median(sigma_Qfit_real); %median fitting flux uncertainty
%         median_sigma_Qsum_real_analysis{i}(m) = median(sigma_Qsum_real); %median summation flux uncertainty
%         median_sigma_Qsum_positive_analysis{i}(m) = median(sigma_Qsum_positive); %median summation flux uncertainty
%         median_sigma_Qsum_all_analysis{i}(m) = median(sigma_Qsum_all); %median summation flux uncertainty
%         mean_Qfit_real_analysis{i}(m) = mean(Qfit_real); %mean fitting flux
%         mean_Qsum_real_analysis{i}(m) = mean(Qsum_real); %mean summation flux
%         mean_Qsum_all_analysis{i}(m) = mean(Qsum_all); %mean summation flux
%         mean_sigma_Qfit_real_analysis{i}(m) = mean(sigma_Qfit_real); %mean fitting flux uncertainty
%         mean_sigma_Qsum_real_analysis{i}(m) = mean(sigma_Qsum_real); %mean summation flux uncertainty
%         mean_sigma_Qsum_all_analysis{i}(m) = mean(sigma_Qsum_all); %mean summation flux uncertainty
