...
for i = 1:N_Intervals
    ...
    try
        ...
        indices_StartTime = find(InstrumentData(i).t>=StartTime);
    end
    
    try
        indices_EndTime = find(InstrumentData(i).t.(TimeSubvariable)<=EndTime);
    catch
        indices_EndTime = find(InstrumentData(i).t<=EndTime);
    end
    ...
    if ~isempty(indices_Interval)
        ...
        try
            ...
            IntervalTimes = [IntervalTimes; InstrumentData(i).t(indices_Interval)]; %add to list of interval times
        end
    end
end
