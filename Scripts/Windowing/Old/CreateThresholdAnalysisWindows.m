...
%% load 30-minute flux law data, rename values
folder_LoadData_FluxLaw = '../../AnalysisData/FluxLaw/'; %folder for retrieving processed data
LoadData_FluxLaw_Path = strcat(folder_LoadData_FluxLaw,'FluxLawWindows_30min_Thresholds.mat'); %path for loading flux law windows
load(LoadData_FluxLaw_Path);
fQ_30_all = fQ_all;
zs_30_all = zs_all;
theta_30_all = theta_all;
zL_30_all = zL_all;
tau_30_all = tauRe_all;

%% load data 
folder_LoadData_1 = '../../AnalysisData/Windowing/'; %folder for retrieving processed data
LoadData_1_Path = strcat(folder_LoadData_1,'TimeWindows_30min_Thresholds.mat'); %path for loading time window data
load(LoadData_1_Path);
folder_LoadData_2 = '../../../../Google Drive/Data/AeolianFieldwork/Processed/'; %folder for retrieving processed data
LoadData_2_Path = strcat(folder_LoadData_2,'DataWindows_30min_Thresholds.mat'); %path for loading full data windows
load(LoadData_2_Path);
folder_LoadData_3 = '../../../../Google Drive/Data/AeolianFieldwork/Processed/'; %folder for retrieving processed data
LoadData_3_Path = strcat(folder_LoadData_3,'WindProfiles_30min_Thresholds.mat'); %path for loading wind profile windows
...
zL_all = cell(N_Sites,1); %stability array
...
u_all = cell(N_Sites,1); %horizontal wind timeseries array
w_all = cell(N_Sites,1); %vertical wind timeseries array
n_all = cell(N_Sites,1); %counts timeseries array
hasfluxdata_all = cell(N_Sites,1); %array of which points have flux data

%profile variables
u_profile_all = cell(N_Sites,1); %profile of horizontal wind speed
zU_profile_all = cell(N_Sites,1); %anemometer heights for profile
zs_profile_all = cell(N_Sites,1); %roughness height from profile
sigma_zs_profile_all = cell(N_Sites,1); %uncertainty in roughness height from profile
ust_profile_all = cell(N_Sites,1); %shear velocity from profile
sigma_ust_profile_all = cell(N_Sites,1); %uncertainty in shear velocity from profile
theta_profile_all = cell(N_Sites,1); %wind direction array
...
for i = 1:N_Sites
    ...
    zL_all{i} = cell(N_Deltat,N_deltat); %stability array
    ...
    w_all{i} = cell(N_Deltat,N_deltat); %vertical wind timeseries array
    n_all{i} = cell(N_Deltat,N_deltat); %counts timeseries array
    hasfluxdata_all{i} = cell(N_Deltat,N_deltat); %array of which points have flux data

    %profile variables - only variation in treatment is by measurement interval
    u_profile_all{i} = cell(N_Deltat,1); %profile of horizontal wind speed
    zU_profile_all{i} = cell(N_Deltat,1); %anemometer heights for profile
    zs_profile_all{i} = cell(N_Deltat,1); %roughness height from profile
    sigma_zs_profile_all{i} = cell(N_Deltat,1); %uncertainty in roughness height from profile
    ust_profile_all{i} = cell(N_Deltat,1); %shear velocity from profile
    sigma_ust_profile_all{i} = cell(N_Deltat,1); %uncertainty in shear velocity from profile
    theta_profile_all{i} = cell(N_Deltat,1); %wind direction array
    
    %% initialize subwindow values
    for m = 1:N_Deltat
        for s = 1:N_deltat
            fD_all{i}{m,s} = zeros(N_subwindows(m),1)*NaN; %Wenglor detection frequency array
            fQ_all{i}{m,s} = zeros(N_subwindows(m),1)*NaN; %Wenglor transport frequency array
            lambda_all{i}{m,s} = zeros(N_subwindows(m),1)*NaN; %estimated particle arrival rate array
            zU_all{i}{m,s} = zeros(N_subwindows(m),1)*NaN; %anemometer height array
            zL_all{i}{m,s} = zeros(N_subwindows(m),1)*NaN; %stability array
            uth_all{i}{m,s} = zeros(N_subwindows(m),1)*NaN; %effective threshold wind
            ustth_all{i}{m,s} = zeros(N_subwindows(m),1)*NaN; %effective threshold shear velocity
            tauth_all{i}{m,s} = zeros(N_subwindows(m),1)*NaN; %effective threshold stress
            ...
            w_all{i}{m,s} = cell(N_subwindows(m),1); %vertical wind timeseries array
            n_all{i}{m,s} = cell(N_subwindows(m),1); %counts timeseries array
            hasfluxdata_all{i}{m,s} = zeros(N_subwindows(m),1); %array of which points have flux data (0=no flux data [default], 1=has flux data)
        end
        
        %profile variables
        u_profile_all{i}{m} = cell(N_subwindows(m),1); %profile of horizontal wind speed
        zU_profile_all{i}{m} = cell(N_subwindows(m),1); %anemometer heights for profile
        zs_profile_all{i}{m} = zeros(N_subwindows(m),1)*NaN; %roughness height from profile
        sigma_zs_profile_all{i}{m} = zeros(N_subwindows(m),1)*NaN; %uncertainty in roughness height from profile
        ust_profile_all{i}{m} = zeros(N_subwindows(m),1)*NaN; %shear velocity from profile
        sigma_ust_profile_all{i}{m} = zeros(N_subwindows(m),1)*NaN; %uncertainty in shear velocity from profile
        theta_profile_all{i}{m} = cell(N_subwindows(m),1); %wind direction array
    end
    
    %% GO THROUGH 30-MINUTE WINDOWS
    for j = 1:N_windows
        
        %% get anemometer height and stability
        zU = zU_window{i}(j); %anemometer height
        zL = zL_30_all{i}(j); %stability
        
        %% ACTIVITY ANALYSIS
        if ~isnan(zW_window{i}{j}) %only perform calculation if there is flux data in window
            ...
            for s = 1:N_deltat
                ...
                for m = 1:N_Deltat
                    ...
                    for k = 1:subwindow_per_window(m)
                        ...
                        w_subwindow = w_deltat(window_ind_subwindow); %vertical winds for subwindow
                        ...
                        w_subwindow_noerr = w_deltat(ind_subwindow_noerr); %vertical winds for subwindow
                        ...
                        zL_all{i}{m,s}(array_ind_subwindow) = zL; %stability - from 30-minute window
                        ...
                        w_all{i}{m,s}{array_ind_subwindow} = w_subwindow; %vertical wind timeseries array
                        n_all{i}{m,s}{array_ind_subwindow} = n_subwindow; %counts timeseries array
                        hasfluxdata_all{i}{m,s}(array_ind_subwindow) = 1; %indicate in array that this subwindow does contain flux data
                    end
                end
            end
        end
        ...
        %% get needed profile values for analysis
        zU_profile_window = zU_profile{i}(j,:); %get anemometer heights
        u_profile_window = cell(1,length(zU_profile_window)); %initialize cell array of wind speeds
        v_profile_window = cell(1,length(zU_profile_window)); %initialize cell array of wind speeds
        t_profile_window = cell(1,length(zU_profile_window)); %initialize cell array of times
        for h = 1:length(zU_profile_window) %go through anemometer heights
            u_profile_window{h} = u_profile{i}{j,h}; %get wind speeds for this height - only non-error points
            v_profile_window{h} = v_profile{i}{j,h}; %get wind speeds for this height - only non-error points
            t_profile_window{h} = t_wind_profile{i}{j,h}; %get times for this height - only non-error points
        end
        ...
        for m = 1:N_Deltat
            ...
            for k = 1:subwindow_per_window(m)
                ...
                %initialize u_bar profile
                u_bar_profile = zeros(1,length(zU_profile_window));
                
                %initialize raw wind angle profile
                theta_profile_raw = zeros(1,length(zU_profile_window));
                
                %% got through anemometers, get mean wind and angle for each
                for h = 1:length(zU_profile_window) %go through anemometer heights
                    window_ind_subwindow = find(t_profile_window{h}>=StartTimes_Deltat(k)&t_profile_window{h}<EndTimes_Deltat(k)); %get indices of all subwindow points within window
                    u_bar_profile(h) = mean(u_profile_window{h}(window_ind_subwindow)); %determine mean wind
                    theta_profile_raw(h) = atan(mean(v_profile_window{h}(window_ind_subwindow)./u_profile_window{h}(window_ind_subwindow)))*180/pi; %wind angle
                end
                
                %correct wind angle profile
                theta_profile_corrected = theta_profile{i}(j,:)+theta_profile_raw;
                
                %calculate roughness height from profile
                [ust,zs,sigma_ust,sigma_zs] = uz_profilefit(u_bar_profile, zU_profile_window);
                
                %add to arrays of values
                u_profile_all{i}{m}{array_ind_subwindow} = u_bar_profile; %wind timeseries array
                zU_profile_all{i}{m}{array_ind_subwindow} = zU_profile_window; %anemometer height - from 30-minute window
                zs_profile_all{i}{m}(array_ind_subwindow) = zs; %roughness height - from profile    
                sigma_zs_profile_all{i}{m}(array_ind_subwindow) = sigma_zs; %uncertainty in roughness height from profile
                ust_profile_all{i}{m}(array_ind_subwindow) = ust; %shear velocity from profile
                sigma_ust_profile_all{i}{m}(array_ind_subwindow) = sigma_ust; %uncertainty in shear velocity from profile
                theta_profile_all{i}{m}{array_ind_subwindow} = theta_profile_corrected; %wind angle
            end
        end
    end
end
...
starttimes_subwindows = starttimes_subwindows(hasfluxdata_all{ind_Site}{ind_Deltat,ind_deltat}==1); %include only times with flux data
endtimes_subwindows = endtime_all{ind_Site}{ind_Deltat,ind_deltat}; %end times for contained subwindows
endtimes_subwindows = endtimes_subwindows(hasfluxdata_all{ind_Site}{ind_Deltat,ind_deltat}==1); %include only times with flux data
