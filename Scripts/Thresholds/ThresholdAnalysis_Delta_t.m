...
LoadTimeData_Path = strcat(folder_LoadData,'TimeWindows_30min.mat'); %path for loading time windows

%% all possible subwindow times
%delta_t_subwindow_all = [duration(0,0,25),duration(0,1,0),duration(0,3,0),duration(0,5,0),duration(0,10,0),duration(0,30,0)]; %all subwindow times for analysis
delta_t_subwindow_all = [duration(0,0,25),duration(0,1,0),duration(0,3,0),duration(0,5,0),duration(0,30,0)]; %all subwindow times for analysis
delta_t_subwindow_primary = duration(0,1,0); %primary subwindow time for analysis
N_delta_t_subwindow = length(delta_t_subwindow_all);
ind_delta_t_subwindow_primary = find(seconds(delta_t_subwindow_all)==seconds(delta_t_subwindow_primary));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% perform sensitivity analysis for subwindow size %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% initializing
% use only Oceano data
i = 3;
zU = 0.64; %anemometer height = 0.64 m

%initialize binned values
fQ_avg_bin_avg_subwindow = cell(N_delta_t_subwindow,1); %transport activity - average
fQ_avg_bin_SE_subwindow = cell(N_delta_t_subwindow,1); %transport activity - standard error
uth_avg_bin_avg_subwindow = cell(N_delta_t_subwindow,1); %TFEM threshold wind - average
uth_avg_bin_SE_subwindow = cell(N_delta_t_subwindow,1); %TFEM threshold wind - standard error
ustth_avg_bin_avg_subwindow = cell(N_delta_t_subwindow,1); %TFEM threshold shear velocity - average
ustth_avg_bin_SE_subwindow = cell(N_delta_t_subwindow,1); %TFEM threshold shear velocity - standard error
tauth_avg_bin_avg_subwindow = cell(N_delta_t_subwindow,1); %TFEM threshold shear stress - average
tauth_avg_bin_SE_subwindow = cell(N_delta_t_subwindow,1); %TFEM threshold shear stress - standard error

%initialize best fit values
tauft_avg_subwindow = zeros(N_delta_t_subwindow,1); %inferred fluid threshold stress - best fit
sigma_tauft_avg_subwindow = zeros(N_delta_t_subwindow,1); %inferred fluid threshold stress - uncertainty
tauit_avg_subwindow = zeros(N_delta_t_subwindow,1); %inferred impact threshold stress - best fit
sigma_tauit_avg_subwindow = zeros(N_delta_t_subwindow,1); %inferred impact threshold stress - uncertainty
ustitftratio_avg_subwindow = zeros(N_delta_t_subwindow,1); %ratio of impact and fluid threshold shear velocities - best fit
sigma_ustitftratio_avg_subwindow = zeros(N_delta_t_subwindow,1); %ratio of impact and fluid threshold shear velocities - uncertainty

%% go through each subwindow duration
for m = 1:N_delta_t_subwindow
    
    %get subwindow duration
    delta_t_subwindow = delta_t_subwindow_all(m);
    
    %Load window data
    LoadActivityData_Path = strcat(folder_LoadData,'ActivityWindows_30min_',int2str(seconds(delta_t_subwindow)),'s.mat'); %path for loading window data
    load(LoadActivityData_Path);
    rho_a = [1.16, 1.22, 1.22];
    ...
    fQ_avg_bin_avg_subwindow{m} = fQ_bin_avg;
    fQ_avg_bin_SE_subwindow{m} = fQ_bin_SE;

    %initialize binned threshold values
    uth_avg_bin_avg_subwindow{m} = zeros(N_fQ_bins,1);
    uth_avg_bin_SE_subwindow{m} = zeros(N_fQ_bins,1);
    ustth_avg_bin_avg_subwindow{m} = zeros(N_fQ_bins,1);
    ustth_avg_bin_SE_subwindow{m} = zeros(N_fQ_bins,1);
    tauth_avg_bin_avg_subwindow{m} = zeros(N_fQ_bins,1);
    tauth_avg_bin_SE_subwindow{m} = zeros(N_fQ_bins,1);

    for k=1:N_fQ_bins
        %generate binned threshold values
        fQ_bin_ind = find(fQ_avg_binning>=fQ_bin_min(k)&fQ_avg_binning<=fQ_bin_max(k));
        uth_avg_bin_avg_subwindow{m}(k) = mean(uth_avg_binning(fQ_bin_ind));
        uth_avg_bin_SE_subwindow{m}(k) = std(uth_avg_binning(fQ_bin_ind))/sqrt(length(fQ_bin_ind));

        %convert from uth to ustth and tauth
        ustth_avg_bin_avg_subwindow{m}(k) = (kappa*uth_avg_bin_avg_subwindow{m}(k))/log(zU/z0); %threshold shear velocity - avg
        ustth_avg_bin_SE_subwindow{m}(k) = (kappa*uth_avg_bin_SE_subwindow{m}(k))/log(zU/z0); %threshold shear velocity - uncertainty
        tauth_avg_bin_avg_subwindow{m}(k) = rho_a(i)*ustth_avg_bin_avg_subwindow{m}(k).^2; %threshold shear stress - avg
        tauth_avg_bin_SE_subwindow{m}(k) = 2*rho_a(i)*ustth_avg_bin_avg_subwindow{m}(k)*ustth_avg_bin_SE_subwindow{m}(k); %threshold shear stress - uncertainty
    end

    %get best fit values
    [a, b, sigma_a, sigma_b, yfit, sigma_yfit, sigma2_ab, da_dy, db_dy] = linearfit(fQ_avg_bin_avg_subwindow{m}, tauth_avg_bin_avg_subwindow{m}, tauth_avg_bin_SE_subwindow{m});
    tauft_avg_subwindow(m) = a;
    sigma_tauft_avg_subwindow(m) = sigma_a;
    tauit_avg_subwindow(m) = a+b;
    sigma_tauit_avg_subwindow(m) = sqrt(sigma_a^2+sigma_b^2+2*sigma2_ab);

    %get ratio of ustit / ustft
    ustitftratio_avg_subwindow(m) = sqrt(tauit_avg_subwindow(m)/tauft_avg_subwindow(m));
    sigma_ustitftratio_avg_subwindow(m) = (1/2)*sqrt((sigma_tauft_avg_subwindow(m)^2/(tauft_avg_subwindow(m)*tauit_avg_subwindow(m)))+...
        (sigma_tauit_avg_subwindow(m)^2*tauft_avg_subwindow(m)/tauit_avg_subwindow(m)^3));
end
...
%plot average values
for m = 1:N_delta_t_subwindow
    plot(fQ_avg_bin_avg_subwindow{m},tauth_avg_bin_avg_subwindow{m},PlotMarkers{m},'Color',PlotColors{m});
end

%plot SE values
for m = 1:N_delta_t_subwindow
    N_fQ_bins = length(fQ_avg_bin_avg_subwindow{m});
    for k = 1:N_fQ_bins
        plot(fQ_avg_bin_avg_subwindow{m}(k)+fQ_avg_bin_SE_subwindow{m}(k)*[-1 1],tauth_avg_bin_avg_subwindow{m}(k)*[1 1],'Color',PlotColors{m});
        plot(fQ_avg_bin_avg_subwindow{m}(k)*[1 1],tauth_avg_bin_avg_subwindow{m}(k)+tauth_avg_bin_SE_subwindow{m}(k)*[-1 1],'Color',PlotColors{m});
    end
end

%plot fit lines
for m = 1:N_delta_t_subwindow
    plot([0 1],[tauft_avg_subwindow(m) tauit_avg_subwindow(m)],'Color',PlotColors{m});
end

%generate legend
legend_items = cell(N_delta_t_subwindow,1)
for m = 1:N_delta_t_subwindow
    legend_items{m} = [num2str(minutes(delta_t_subwindow_all(m)),2), ' minutes']
end
...
print([folder_Plots,'tauth_fQ_delta_t_sensitivity.png'],'-dpng');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% for remaining analyses, just use primary subwindow time %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

delta_t_subwindow = delta_t_subwindow_primary;
...
for i = 1:N_Sites
    ...
    if ~isempty(fQ_avg_binning)
        ...
        for k=1:N_fQ_bins
            ...
            tauth_avg_bin_avg{i}(k) = rho_a(i)*ustth_avg_bin_avg{i}(k)^2; %threshold shear stress - avg
            tauth_avg_bin_SE{i}(k) = 2*rho_a(i)*ustth_avg_bin_avg{i}(k)*ustth_avg_bin_SE{i}(k); %threshold shear stress - uncertainty
    ...
    for j=1:N_diurnalrange
        ...
        if ~isempty(fQ_avg_binning_diurnal)
            ...
            for k=1:N_fQ_bins   
                ...
                tauth_avg_diurnal_bin_avg{i}{j}(k) = rho_a(i)*ustth_avg_diurnal_bin_avg{i}{j}(k)^2; %threshold shear stress - avg
                tauth_avg_diurnal_bin_SE{i}{j}(k) = 2*rho_a(i)*ustth_avg_diurnal_bin_avg{i}{j}(k)*ustth_avg_diurnal_bin_SE{i}{j}(k); %threshold shear stress - uncertainty
            end
        end
        ...
        %get ratio of ustit / ustft
        ustitftratio_avg_diurnal(i,j) = sqrt(tauit_avg_diurnal(i,j)/tauft_avg_diurnal(i,j));
        sigma_ustitftratio_avg_diurnal(i,j) = (1/2)*sqrt((sigma_tauft_avg_diurnal(i,j)^2/(tauft_avg_diurnal(i,j)*tauit_avg_diurnal(i,j)))+...
